...
export default function PropertyDetailPage({ property }: PropertyDetailPageProps) {
  ...
  useEffect(() => {
    ...
    async function loadSuggested() {
      try {
        ...
        setSuggested(list.data.filter((item) => item.id !== property.id).slice(0, 4));
      } catch {
        if (!mounted) return;
      } finally {
  ...
  useEffect(() => {
    setActiveImage(pickImage(property));
  }, [property]);
    ...
    <main className="bg-white text-slate-900">
      <header className="sticky top-0 z-50 border-b border-slate-200/70 bg-white/95 backdrop-blur-xl">
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-4 py-2.5 sm:px-6 sm:py-3 lg:px-8">
          ...
          <nav className="hidden items-center gap-8 text-sm font-semibold text-slate-700 lg:flex">
            ...
            <Link href="/properties" className="text-red-600">
              Properties
            </Link>
      ...
      <section className="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
        <div className="mb-4 flex items-center gap-2 text-sm text-slate-500">
          ...
          <span>›</span>
          <Link href="/properties" className="hover:text-red-600">
            Properties
          </Link>
          <span>›</span>
        ...
        <div className="grid gap-5 lg:grid-cols-[1.55fr_0.95fr]">
          <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
            <div className="relative h-[270px] overflow-hidden rounded-xl bg-slate-100 sm:h-[460px]">
              <Image
                src={activeImage || pickImage(property)}
                alt={property.title}
                fill
              ...
              {gallery.map((url) => (
                <button
                  ...
                  className={`h-16 overflow-hidden rounded-lg border ${
                    activeImage === url ? 'border-red-500' : 'border-slate-200'
                  }`}
                >
          ...
          <aside className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
            ...
            <h1 className="mt-3 text-[2.05rem] font-bold tracking-[-0.03em] text-slate-900 sm:text-4xl">
              {property.title}
            </h1>
        ...
        <article className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
          ...
          <p className="mt-3 whitespace-pre-line text-sm leading-7 text-slate-600">
            {property.description ||
              'A strategic land investment opportunity with ready documentation and excellent growth potential.'}
          </p>
        </article>
              ...
              {visitFeedback ? (
                <p
                  className={`rounded-md px-3 py-2 text-xs ${
                    visitFeedback.type === 'success' ? 'bg-emerald-50 text-emerald-700' : 'bg-rose-50 text-rose-700'
                  }`}
                >
